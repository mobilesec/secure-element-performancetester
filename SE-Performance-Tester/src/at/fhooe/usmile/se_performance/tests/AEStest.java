...
package at.fhooe.usmile.se_performance.tests;

import java.util.Arrays;

import at.fhooe.usmile.se_performance.TestPerformerActivity;
import at.fhooe.usmile.se_performance.connection.SEConnection;
import at.fhooe.usmile.se_performance.util.APDU;
import at.fhooe.usmile.se_performance.util.Converter;

public class AEStest {

	private final static byte CLA = (byte) 0x80;

	private final static byte INS_ENCRYPT128_TEST = 0x41;
	private final static byte INS_DECRYPT128_TEST = 0x42;

	private final static byte INS_ENCRYPT256_TEST = 0x45;
	private final static byte INS_DECRYPT256_TEST = 0x46;
	...
	public AEStest(TestPerformerActivity _testerInstance, SEConnection _seConn,
			int _keyLen) {
	...
	public void perfomanceSummary() {
		...
		testerInstance.logText("\n--AES " + String.valueOf(keyLen) + "---Performance Summary ---- iterations : "
				+ count);
		testerInstance
				.logText("\n==================================================");
	...
	public byte[] encryptTest(int testnum, byte[] _data) {
		...
		byte[] cmdApdu;
		if (keyLen == 128) {
			cmdApdu = APDU.getCommandApdu(CLA, INS_ENCRYPT128_TEST, P1, P2,
					_data, Le);
		} else {
			cmdApdu = APDU.getCommandApdu(CLA, INS_ENCRYPT256_TEST, P1, P2,
					_data, Le);
		}
	...
	public void decryptTest(int testnum, byte[] _testData) {
		byte[] cmdApdu;
		if (keyLen == 128) {
			testerInstance.logText("\nDecrypt Test " + testnum
					+ "\n--------------------------------------------------");
			cmdApdu = APDU.getCommandApdu(CLA, INS_DECRYPT128_TEST, P1, P2,
					_testData, Le);
		} else {
			cmdApdu = APDU.getCommandApdu(CLA, INS_DECRYPT256_TEST, P1, P2,
					_testData, Le);
		}
		testerInstance.logText("\nDecrypt Test " + testnum
				+ "\n--------------------------------------------------");
